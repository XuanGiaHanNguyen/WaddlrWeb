...
import { StepHeader, Divider, FormGrid, FieldGroup, Label, Input, Select, RadioGroup, CheckGroup, PillGroup, Counter } from "../components/primitives";
    ...
    <div style={{ animation: "fadeUp 0.4s ease both" }}>
      ...
      <FormGrid>
        ...
        <FieldGroup>
          ...
          <Select>{["Ground floor", "1st floor", "2nd floor +"].map((o) => <option key={o}>{o}</option>)}</Select>
        </FieldGroup>
        <FieldGroup span2><Label hint="Private — never shared publicly.">Full address</Label><Input placeholder="123 Maplewood Dr, Tampa, FL 33602" /></FieldGroup>
        <FieldGroup span2><Label>Brief description of the space</Label><Input placeholder="e.g. Quiet private room with lock, own bathroom, close to bus stops…" /></FieldGroup>
      </FormGrid>
          ...
          {DAYS.map((d) => {
            ...
            return (
              <button
                key={d}
                onClick={() => toggleDay(d)}
                style={{ padding: "0.5rem 0.2rem", borderRadius: 10, border: `1.5px solid ${on ? C.safeGreen : "rgba(74,106,120,0.2)"}`, background: on ? C.safeGreen : "transparent", color: on ? C.white : C.muted, fontSize: "0.72rem", fontWeight: 600, cursor: "pointer", transition: "all 0.2s", textAlign: "center" }}
              >
                {d}
              </button>
            );
      ...
      <FormGrid>
        ...
        <FieldGroup span2>
          ...
          <Select>{["Immediately (I'm on-call)", "30 minutes", "1 hour", "2–4 hours", "Same day", "24 hours"].map((o) => <option key={o}>{o}</option>)}</Select>
        </FieldGroup>
      </FormGrid>
